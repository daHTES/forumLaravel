<template>
    <div>
        <div class="flex items-center mb-6">
            <h3>Разделы</h3>
            <template v-if="checkGlobalAuth(section)">
                <Link :href="route('sections.create')" class="block mr-4 w-1/6 px-2 py-1 bg-white border-gray-300 rounded-lg text-center">+ Раздел</Link>
            </template>
            <template v-if="sections.filter(section => checkSectionAuth(section)).length">
                <Link :href="route('branches.create')" class="block w-1/6 px-2 py-1 bg-white border-gray-300 rounded-lg text-center">+ Ветка</Link>
            </template>

        </div>
        <div v-if="sections">
            <div v-for="section in sections" class="mb-6">
