<template>
    <div>
        <div class="flex items-center mb-6">
            ...
            <Link :href="route('admin.roles.create')" class="block mr-4 w-1/6 px-2 py-1 bg-white border-gray-300 rounded-lg text-center">+ Роль</Link>
        </div>
        <div>
                ...
                <table class="text-center w-full text-medium">
                    ...
                    <tr v-for="user in users" class="text-gray-500">
                        ...
                        <td class="p-4 relative">
                            ...
                            <div class="z-10 text-left absolute left-0 bg-white p-4 border border-gray-300" v-if="user.is_role_change">
                                    <div v-for="role in roles" class="mb-2">
                                            <input @change="toggleRole(user, role.id)" :checked="user.roles.some( userRole => userRole.id === role.id)" class="mr-2" type="checkbox" :id="`${role.id} ${role.id}`" :value="role.id">
                                            <label class="cursor-pointer" :for="`${role.id} ${role.id}`">{{ role.code }}</label>
                                    </div>
                            </div>
                        </td>
                    </tr>
