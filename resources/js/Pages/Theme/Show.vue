        ...
        <div v-if="theme.messages.length">
            <div v-for="message in theme.messages" :id="message.id" class="flex bg-white border border-gray-300">
                    <div class="p-4 w-1/6 border-r border-gray-300">
                        <div class="w-24 h-24 bg-gray-300 overflow-hidden rounded-full mx-auto mb-2">
                ...
                <div class="p-4 w-5/6">
                    <div class="mb-2">
                        <p class="text-sm italic"> {{ message.time }}
                        </p>
                    </div>
                    <div>
                        <div class="mb-4" v-if="message.is_not_solved_complaint">
                            <p class="w-full bg-red-100 p-2 border border-red-200">Ваша жалоба в рассмотрении</p>
                        </div>
                        <div class="mb-4">
                        ...
                        <div class="flex items-center justify-end">
                            ...
                            <div class="mr-4 mb-4">
                                <a @click.prevent="openComplaint(message)" href="#" class="text-sm rounded-lg bg-white border border-red-800 inline-block py-2 px-3 text-center text-red-800">Пожаловаться</a>
                            </div>

                            <div class="mr-4 mb-4">
                                <a @click.prevent="quote(message.content)" href="#" class="text-sm rounded-lg bg-sky-600 border border-sky-700 inline-block py-2 px-3 text-center text-white">Цитировать</a>
                            </div>

                            <div class="mr-4 mb-4">
                                <a @click.prevent="answer(message)" href="#" class="text-sm rounded-lg bg-indigo-600 border border-indigo-700 inline-block py-2 px-3 text-center text-white">Ответить</a>
                            </div>
                        ...
                        <div class="flex" v-if="message.is_complaint">
                            <input v-model="message.body" type="text" placeholder="Ваша жалоба" class="rounded-r-none p-2 w-5/6 rounded-lg border border-gray-300 w-full">
            ...
            <div class="mb-4">
                <div class="bg-gray-50 flex items-center border border-gray-100 p-2 w-full">
                    ...
                    <div ref="editor" class="w-full border border-gray-300 p-4" contenteditable="true">
                    </div>
            </div>
                <div>
                    <a @click.prevent="store" class="block w-1/4 p-2 bg-sky-600 text-white text-center border border-sky-700" href="#">Добавить сообщение</a>
                </div>
            </div>
    </div>
</template>
